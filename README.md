...
### 📸 Статичный вид части компонентов курсовой работы

![Статичный вид](screenshots/screen1-static.png)

> _Отображение компонента выбора количества гостей, поля поиска, карточек помещений_

### 📸 Интерактив: компонент выбора количества гостей

![Интерактив](screenshots/screen3-state.png)

> _Работа useState: изменение количества гостей от 1 до 10_

### 📸 Интерактив: умный input

![Фильтрация](screenshots/screen2-list.png)

> _Поиск по названию помещения с обновлением списка карточек помещений в реальном времени_

---

### 📦 Реализованные компоненты

В рамках текущей практической работы реализованы следующие компоненты:

#### 🔹 Core (Корневые)

- **`App.js`** — Точка входа и сборщик интерфейса.
  - Хранит массив данных `SERVICES`.
  - Управляет состоянием фильтрации (`query`).
  - Реализует логику поиска (фильтрация массива перед рендерингом).
  - Передает данные в дочерние компоненты через `props`.

#### 🔹 UI & Logic (Компоненты интерфейса)

- **`ServiceCard.jsx`** — Карточка помещения (Presentational Component).
  - Принимает данные через `props` (`serviceName`, `specialist`, `price`, `badge`).
  - Отвечает только за отображение информации (без собственной логики состояния).
  - Реализует условный рендеринг бейджа (показывает, только если `badge` передан).

- **`ServiceSearch.jsx`** — Поле поиска (Controlled Component).
  - Принимает `value` и `onChange` от родителя.
  - Является "глупым" компонентом (не хранит стейт самостоятельно).
  - Обеспечивает мгновенную фильтрацию списка.

- **`GuestSelector.jsx`** — Выбор количества участников (Smart Component).
  - Использует `useState` для хранения количества гостей (`guestsCount`).
  - Реализует бизнес-логику валидации: минимум 1, максимум 10 гостей.
  - Содержит кнопки управления (`+`, `-`, `Сброс`).
  - Реализует функциональное обновление стейта (`prev => prev + 1`).

#### 🔹 Stubs (Заглушки под курсовой проект)

- **`TimeSlot.jsx`** — Плейсхолдер для будущего выбора времени бронирования.
- **`BookingForm.jsx`** — Плейсхолдер для формы подтверждения бронирования.
